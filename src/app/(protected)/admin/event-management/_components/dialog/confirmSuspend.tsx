    ...
    <Dialog open={open} onClose={onClose}>
      <div className="text-white dialog-header px-6 py-4 justify-center items-center flex relative" style={{ background: '#0C4762' }}>
        <DialogTitle className="!m-0 !p-0 text-lg text-center font-bold">
          {transWithFallback('confirmSuspend', 'Xác nhận đình chỉ sự kiện')}
        </DialogTitle>
        ...
        {isLoading ? (
          ...
          <>
            ...
            <div className="content mx-4 mt-2 mb-4 text-center">
              <p>{transWithFallback('uSureWantToChangeStatusEvent', 'Bạn có chắc muốn chuyển trạng thái sự kiện này sang ')}</p>
              <p><strong>&quot; {transWithFallback('waitingToApprove', 'Chờ duyệt')} &quot;</strong> ?</p>
            </div>
