...
<div className="w-full flex flex-col gap-4">
  {/* Header Section */}
  <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 pt-6">
    {/* Search Bar */}
    <div className="flex items-center border border-gray-300 rounded-md overflow-hidden w-full lg:w-1/3 bg-white">
      <input
        type="text"
    ...
    <div className="flex flex-wrap gap-3 w-full lg:w-auto">
      <button
        ...
        className="px-4 py-2 text-sm bg-[#48D1CC] text-[#0C4762] rounded-md transition duration-200 hover:bg-[#51DACF] w-full sm:w-auto"
      >
        {transWithFallback("sendAllEmails", "Gửi tất cả email")}
      </button>
      <button
        onClick={handleSendSelectedEmail}
        className={`px-4 py-2 text-sm rounded-md transition duration-200 w-full sm:w-auto ${
  ...
  <div className="overflow-x-auto">
    <table className="min-w-full border text-sm">
      <thead>
        <tr className="bg-[#0C4762] text-white text-left whitespace-nowrap">
      ...
      <tbody>
        {ordersData.length > 0 ? (
          ordersData.map((order) => {
            ...
            return (
              <tr key={order.id} className="border-t hover:bg-gray-200 cursor-pointer">
