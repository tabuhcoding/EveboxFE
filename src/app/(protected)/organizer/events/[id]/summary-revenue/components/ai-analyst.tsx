...
export function AIAnalyst({ showingId }: AIAnalystProps) {
  ...
  useEffect(() => {
    const fetchStoredResponses = async () => {
      setLoading(true);
        try {
          const storedResponses = await axios.get(`${process.env.NEXT_PUBLIC_API_URL!}/api/org/statistics/summary-ai/${showingId}`,{
          params: { page, limit }
        });
  ...
  return (
    <div className="mt-8">
      <h3 className="text-lg font-bold text-[#0C4762] mb-2">
        {transWithFallback('AIRevenueAnalystWithAI', 'Phân tích doanh thu của bạn với AI')}
      </h3>
