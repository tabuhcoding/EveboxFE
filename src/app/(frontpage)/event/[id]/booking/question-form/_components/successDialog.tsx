...
export default function SuccessDialog({ message, onClose, open, href }: AlertDialogProps) {
      ...
      <div className="text-white dialog-header px-6 py-4 justify-center items-center flex relative" style={{ background: '#0C4762' }}>
        ...
        {!href && (
          <button onClick={onClose} className="absolute right-2 top-2 px-1 py-1 close-btn">
            <Icon icon="icons8:checked" width="20" height="20" />
          </button>
        )}
      </div>
      ...
      <DialogContent className="p-6 flex flex-col justify-center items-center">
        ...
        {href && (
          <button onClick={handleRouterPush} className="bg-[#0C4762] hover:bg-[#1d3945] text-white font-bold py-2 px-4 rounded mt-4">
            {transWithFallback('registerTicketSuccess', 'Bạn đã đăng ký vé thành công! Tiếp tục đến trang Vé của tôi để xem chi tiết')}
          </button>
        )}
      </DialogContent>
    </Dialog>
